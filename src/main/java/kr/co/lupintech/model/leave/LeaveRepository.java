...
public interface LeaveRepository extends JpaRepository<Leave, Long> {
    ...
    @Query("select l from Leave l where l.type = :duty " +
            "and l.startDate = :date and l.user.id = :id")
    List<Leave> findDuplicateDuty(@Param("duty") LeaveType type, @Param("date") LocalDate startDate, @Param("id") Long userId);
    ...
    @Query("select l from Leave l where l.type = :annual and l.user.id = :id " +
            "and ((l.startDate <= :start and l.endDate >= :start) or (l.startDate <= :end and l.endDate >= :end)" +
            "or (l.startDate >= :start and l.endDate <= :end))")
    List<Leave> findDuplicateAnnual(@Param("annual") LeaveType type, @Param("start") LocalDate startDate,
                                    @Param("end") LocalDate endDate, @Param("id") Long userId);
